...
public class SimDetailInfoActivity extends BaseActivity<SimDetailInfoPresenter> implements SimDetailInfoContract.View {
    ...
    private void showDetailView(SimDetailResultBean simDetailResultBean){
        ...
        DecimalFormat df = new DecimalFormat("#.00");//CHANGE_DO
        tvRemainingFlow.setText(df.format(((double) (simDetailResultBean.getTflow() - simDetailResultBean.getRflow()) / 100)) + "");
        tvFlowTotal.setText(df.format(((double) simDetailResultBean.getTflow() / 100)) + "M");
        tvFlowUse.setText(df.format(((double) simDetailResultBean.getRflow() / 100)) + "M");
