...
import com.car.scth.mvp.model.putbean.OilSetPutBean;
 ...
 */
@ActivityScope
public class OilElectricityControlModel extends BaseModel implements OilElectricityControlContract.Model{
    @Inject
    Gson mGson;
    @Inject
    ...
    @Inject
    public OilElectricityControlModel(IRepositoryManager repositoryManager) {
    ...
    @Override
    public Observable<BaseBean> submitOilElectricControl(OilSetPutBean bean) {
        Gson gson = new Gson();
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), gson.toJson(bean));
        return Observable.just(mRepositoryManager.obtainRetrofitService(DeviceService.class).submitOilElectricControl(ConstantValue.getApiUrlSid(), requestBody))
                .flatMap(new Function<Observable<BaseBean>, ObservableSource<BaseBean>>() {
                    ...
                    public ObservableSource<BaseBean> apply(Observable<BaseBean> baseBeanObservable) throws Exception {
                        return mRepositoryManager.obtainRetrofitService(DeviceService.class).submitOilElectricControl(ConstantValue.getApiUrlSid(), requestBody);
                    }
                });
    }

    @Override
    public Observable<BaseBean> submitModifyPassword(ModifyPasswordPutBean bean) {
        Gson gson = new Gson();
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), gson.toJson(bean));
