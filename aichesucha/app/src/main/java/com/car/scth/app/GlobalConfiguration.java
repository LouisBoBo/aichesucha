...
import com.car.scth.BuildConfig;
...
//                        Timber.e("iccJsonRequest:" + bodyString);
                        new DefaultFormatPrinter().printJsonRequest(request, bodyString);
                    }

                    @Override
                    public void printFileRequest(Request request) {
                        new DefaultFormatPrinter().printFileRequest(request);
//                        Timber.e("iccFileRequest:" + request.url().toString());
                    }
                    ...
                    public void printJsonResponse(long chainMs, boolean isSuccessful, int code,
                        ...
                        Timber.e("moneyResponse:" + bodyString);
//                        new DefaultFormatPrinter().printJsonResponse(chainMs, isSuccessful, code, headers, contentType, bodyString, segments, message, responseUrl);
                    }

                    @Override
                    public void printFileResponse(long chainMs, boolean isSuccessful, int code, String headers,
                                                  List<String> segments, String message, String responseUrl) {
                        Timber.e("moneyFileResponse:" + responseUrl);
//                        new DefaultFormatPrinter().printFileResponse(chainMs, isSuccessful, code, headers, segments, message, responseUrl);
                    }
                })
...
//                    okhttpBuilder.sslSocketFactory(); //支持 Https, 详情请百度
                    okhttpBuilder.writeTimeout(60, TimeUnit.SECONDS); //写入超时
                    okhttpBuilder.connectTimeout(60, TimeUnit.SECONDS); //连接超时
                    okhttpBuilder.readTimeout(60, TimeUnit.SECONDS); //读取超时
